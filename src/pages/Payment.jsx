...
import {
  HomeOutlined, WorkOutlined, ArrowBackOutlined,
  CreditCardOutlined, AccountBalanceWalletOutlined,
  TwoWheelerOutlined
...
const SAVED_ADDRESSES = [
  { id: '1', label: 'Home', address: 'No 12 Adeleke Street, Bodija, Ibadan', icon: <HomeOutlined fontSize="small" /> },
  { id: '2', label: 'Work', address: 'Suite 4B, Challenge Plaza, Ibadan', icon: <WorkOutlined fontSize="small" /> },
]

const PAYMENT_METHODS = [
  { id: 'card', label: 'Visa ending in 4242', sub: 'Expires 12/26', icon: <CreditCardOutlined /> },
  { id: 'wallet', label: 'App Wallet', sub: '₦0.00 balance', icon: <AccountBalanceWalletOutlined /> },
  { id: 'cash', label: 'Cash on Delivery', sub: 'Pay when your order arrives', icon: <TwoWheelerOutlined /> },
]
...
function Payment() {
  ...
  const handlePlaceOrder = () => {
    const order = placeOrder({
      ...
      total,
      restaurantId,
      restaurantName: items[0]?.restaurantId,
  ...
  if (items.length === 0) {
    return (
      <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '60vh', gap: 2 }}>
        <Typography variant="h6" fontWeight={700}>Nothing to pay for</Typography>
        <Button variant="contained" onClick={() => navigate('/home')}>Browse Restaurants</Button>
      </Box>
    )
  }
              ...
              {SAVED_ADDRESSES.map(addr => (
                <Box
                  ...
                  sx={{
                    ...
                    bgcolor: selectedAddress === addr.id ? 'rgba(230,57,70,0.05)' : 'background.default',
                ...
                >
                  ...
                  <Typography variant="caption" color="text.secondary">{addr.address}</Typography>
                  <FormControlLabel
                    value={addr.id}
                ...
                <Box key={method.id}>
                  <Box
                    ...
                    sx={{
                      ...
                      p: 1.5, borderRadius: 2, cursor: 'pointer',
                      bgcolor: selectedPayment === method.id ? 'rgba(230,57,70,0.05)' : 'transparent',
        ...
        <CardContent>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <Typography variant="body2" fontWeight={600}>₦{deliveryFee.toLocaleString()}</Typography>
            </Box>
            <Divider />
            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
