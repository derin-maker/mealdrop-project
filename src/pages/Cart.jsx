...
function Cart() {
  ...
  if (items.length === 0) {
    return (
      <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "60vh", gap: 2, px: 3 }}>
    ...
    <Box className="page-fade" sx={{ px: { xs: 2, md: 4 }, py: 3 }}>
      ...
      <Box sx={{ display: "flex", gap: 3, alignItems: "flex-start", flexDirection: { xs: "column", md: "row" } }}>
        <Box sx={{ flex: 1, display: "flex", flexDirection: "column", gap: 2 }}>
          ...
          <Button variant="outlined" color="error" startIcon={<DeleteSweepOutlined />}
            onClick={clearCart} sx={{ alignSelf: "flex-start", mt: 1 }}>
            Clear Cart
          </Button>
        </Box>

        <Paper sx={{ p: 3, width: { xs: "100%", md: 320 }, position: { md: "sticky" }, top: { md: 80 }, borderRadius: 3 }}>
          <Typography variant="h6" fontWeight={800} mb={2}>Order Summary</Typography>
